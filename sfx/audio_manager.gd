...
# Call this function when an event happens (e.g., weapon shot, explosion)
##func trigger_sfx_networked(sfx_key: StringName, spawn_position: Vector3) -> void:
##	play_sfx_rpc.rpc(sfx_key, spawn_position)
##
##@rpc("any_peer", "call_local", "reliable")
##func play_sfx_rpc(sfx_key: StringName, spawn_position: Vector3) -> void:
##	if !multiplayer.is_server(): return
##	# Plays locally on every connected peer at the 3D position
##	AudioManager.play_sfx(sfx_key, spawn_position)
